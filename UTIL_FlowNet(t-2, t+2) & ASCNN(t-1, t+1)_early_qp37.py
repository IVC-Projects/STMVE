...
# [[high, low1, ascnn, label1], [[h21,h22], low2, ascnn, label2]]
def get_test_list(HIGHDATA_Parent_PATH, ASCNN_Frames_PATH, lowDataList, labelList):
    doubleTest_list = []
    singleTest_list = []
    HighDirList = os.listdir(HIGHDATA_Parent_PATH)
    ASCNNList = os.listdir(ASCNN_Frames_PATH)
    ...
    for lowdata in lowDataList:
        ...
        if lowdataIndex % 4 != 0:
            if lowdataIndex in HighDirList:
                ...
                tempData.append(os.path.join(ASCNN_Frames_PATH, ASCNNList[ASCNN_Frames_index]))
...
def get_train_list(high1DataList, lowDataList, high2DataList, high3DataList, labelList):
    assert len(lowDataList) == len(high1DataList) == len(labelList) == len(high2DataList) == len(high3DataList), \
        "low:%d, high1:%d, label:%d, high2:%d, high3:%d" % \
        (len(lowDataList), len(high1DataList), len(labelList), len(high2DataList), len(high3DataList))
    ...
    for i in range(len(labelList)):
        ...
        if int(os.path.basename(high1DataList[i]).split('_')[-1].split('.')[0]) == \
                ...
                int(os.path.basename(high3DataList[i]).split('_')[-1].split('.')[0]) + 1:
            tempData.append(high1DataList[i])
            tempData.append(lowDataList[i])
            tempData.append(high2DataList[i])
            tempData.append(high3DataList[i])
            temp.append(tempData)
            temp.append(labelList[i])
...
def prepare_nn_data(train_list):
    ...
    for i in batchSizeRandomList:
        ...
        high3Data_image = c_getYdata(train_list[i][0][3])
        ...
        #crop images to the disired size.
            high1Data_imgY, lowData_imgY, high2Data_imgY, high3Data_imgY, gt_imgY = \
                crop(high1Data_image, lowData_image, high2Data_image, high3Data_image, gt_image, PATCH_SIZE[0], PATCH_SIZE[1], "ndarray")
        ...
        #normalize
            ...
            high3Data_imgY = normalize(high3Data_imgY)
            ...
            high3Data_list.append(high3Data_imgY)
    ...
    high3Data_list = np.resize(high3Data_list, (BATCH_SIZE, PATCH_SIZE[0], PATCH_SIZE[1], 1))
    ...
    return high1Data_list, lowData_list, high2Data_list, high3Data_list, gt_list
...
def crop(high1Data_image, lowData_image, high2Data_image, high3Data_image, gt_image, patch_width, patch_height, img_type):
    assert type(high1Data_image) == type(gt_image) == type(lowData_image)\
           == type(high2Data_image) == type(high3Data_image), "types are different."
    high1Data_cropped = []
    lowData_cropped = []
    high2Data_cropped = []
    high3Data_cropped = []
    ...
    if img_type == "ndarray":
        ...
        high3Data_cropped = high3Data_image[in_row_ind:in_row_ind + patch_width, in_col_ind:in_col_ind + patch_height]
    ...
    return high1Data_cropped, lowData_cropped, high2Data_cropped, high3Data_cropped, gt_cropped
