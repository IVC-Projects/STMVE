...
from Train_ASCNN(t-2, t+2)_early import BATCH_SIZE
from Train_ASCNN(t-2, t+2)_early import PATCH_SIZE
...
def get_test_list(ASCNN_Frames_List, lowDataList, labelList):
    doubleTest_list = []
    singleTest_list = []
...
def get_train_list(high1DataList, lowDataList, labelList):
    assert len(lowDataList) == len(high1DataList) == len(labelList), \
        "low:%d, high1:%d, label:%d" % (len(lowDataList), len(high1DataList), len(labelList))
...
def prepare_nn_data(train_list):
    ...
    for i in batchSizeRandomList:
        ...
        #crop images to the disired size.
            high1Data_imgY, lowData_imgY, gt_imgY = \
                crop(high1Data_image, lowData_image, gt_image, PATCH_SIZE[0], PATCH_SIZE[1], "ndarray")
    ...
    return high1Data_list, lowData_list, gt_list
...
def crop(high1Data_image, lowData_image, gt_image, patch_width, patch_height, img_type):
    assert type(high1Data_image) == type(gt_image) == type(lowData_image), "types are different."
    high1Data_cropped = []
    lowData_cropped = []
    gt_cropped = []
    ...
    return high1Data_cropped, lowData_cropped, gt_cropped
